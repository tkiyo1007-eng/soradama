...
final class TipJar {
    /// 投げ銭の導線を画面に出すかどうか。
    ///
    /// Open-Meteo の無料プランは規約上「商用利用は不可」だが、任意の寄付が
    /// これに当たるかは明記が無かったため、直接問い合わせて確認した
    /// (2026-08-12, Ticket#234081)。回答は次のとおり:
    ///
    ///   寄付が完全に任意であり、何らかの特典や機能の解放につながらない限り、
    ///   非営利アプリとして認められる
    ///
    /// つまり、この条件を満たしている間は無料プランのままでよい。
    /// **投げ銭で何かをアンロックする実装を入れた時点でこの前提は崩れる**ので、
    /// 見返りを付けたくなったときは有料プランへの切り替えとセットで考えること。
